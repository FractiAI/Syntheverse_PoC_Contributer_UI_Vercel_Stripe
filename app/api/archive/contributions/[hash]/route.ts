...
export async function DELETE(request: NextRequest, { params }: { params: { hash: string } }) {
  const submissionHash = params.hash;
  ...
  try {
    ...
    await db.delete(pocLogTable).where(eq(pocLogTable.submission_hash, submissionHash));
