...
export async function POST(
    ...
    try {
        ...
        const qualifiedEpoch = String(metadata.qualified_epoch || 'founder').toLowerCase().trim()
        ...
        if (totalAllocated <= 0) {
            return NextResponse.json({ error: 'No allocatable metal balances available for this PoC' }, { status: 400 })
        }
