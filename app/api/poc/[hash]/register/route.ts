...
export async function POST(request: NextRequest, { params }: { params: { hash: string } }) {
  ...
  try {
    ...
    if (!rateLimitResult.success) {
      debug('RegisterPoC', 'Rate limit exceeded', {
    ...
    if (contrib.registered) {
      ...
      return NextResponse.json({ error: 'PoC is already registered' }, { status: 400 });
    }
    ...
    if (!registrationEnabled) {
      debug('RegisterPoC', 'Vault opens Spring Equinox, March 20, 2026', {
        submissionHash,
        reason:
          'ENABLE_BLOCKCHAIN_REGISTRATION is not set to true - Vault opens Spring Equinox, March 20, 2026',
      });
      return NextResponse.json(
        {
          error:
            'The Vault opens Spring Equinox, March 20, 2026. Blockchain registration will be enabled then.',
          message:
            'SYNTH90T MOTHERLODE VAULT opens Spring Equinox, March 20, 2026. On-chain registration will be available when the vault opens. All qualifying PoCs will be registered on-chain and allocated SYNTH, by score.',
        },
    ...
    if (existingAllocations.length === 0 && isQualified && !isFinancialSupport) {
      try {
        ...
        } = await import('@/utils/tokenomics/epoch-metal-pools');
