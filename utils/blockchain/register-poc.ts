...
export async function registerPoCOnBlockchain(
  ...
  try {
    ...
    if (!config) {
      debugError(
        'RegisterPoCBlockchain',
        'Base blockchain configuration not available',
        new Error('Missing Base configuration')
      );
      return {
        success: false,
    ...
    const metal = (metals && metals.length > 0 ? metals[0] : 'copper') as
      | 'gold'
      | 'silver'
      | 'copper';
    ...
    if (!lensResult.success || !lensResult.transaction_hash) {
      debugError(
        'RegisterPoCBlockchain',
        'Failed to emit Lens event',
        new Error(lensResult.error || 'Unknown error')
      );
      return {
        success: false,
  ...
  try {
    ...
    if (!config) {
      debug(
        ...
        'Base configuration not available, skipping verification'
      );
      return false;
    }
