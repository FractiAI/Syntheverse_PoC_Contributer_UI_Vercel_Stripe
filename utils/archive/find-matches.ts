  ...
  try {
    ...
    const archivedPoCs = archivedPoCsWithLogs
      .map((log) => {
        ...
        const archiveData = metadata?.archive_data as
          | { abstract?: string; formulas?: string[]; constants?: string[] }
          | undefined;
    ...
    const top3 = matches.sort((a, b) => b.similarity_score - a.similarity_score).slice(0, 3);
