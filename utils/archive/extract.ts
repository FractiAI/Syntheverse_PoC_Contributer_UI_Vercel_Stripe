...
export function extractAbstract(textContent: string, title: string): string {
  ...
  if (firstParagraph && firstParagraph.length > 50) {
    return firstParagraph.length > 1000
      ? firstParagraph.substring(0, 1000) + '...'
      : firstParagraph;
  }
...
export function extractFormulas(textContent: string): string[] {
  ...
  const equationPattern =
    /([A-Za-zα-ωΑ-Ω_][A-Za-zα-ωΑ-Ω0-9_^\/×·\s\-\+\*\(\)≈=<>≤≥±∞∑∏∫√]+=+[A-Za-zα-ωΑ-Ω0-9_^\/×·\s\-\+\*\(\)≈<>≤≥±∞∑∏∫√\.]+)/g;
...
export function extractConstants(textContent: string): string[] {
  ...
  const namedConstantPattern =
    /([A-Za-zα-ωΑ-Ω_][A-Za-zα-ωΑ-Ω0-9_^\/×·\s]+[≈=]\s*[\d\.\s×\^10\+\-]+[A-Za-z\/\s]*)/g;
  ...
  const constantSectionPattern =
    /(?:constant|scaling\s+constant|physical\s+constant)[:\s]+([^\n]+)/gi;
  const sectionConstants = content.match(constantSectionPattern);
  if (sectionConstants) {
    sectionConstants.forEach((constant) => {
      const cleaned = constant
        .replace(/^(?:constant|scaling\s+constant|physical\s+constant)[:\s]+/i, '')
        .trim();
      if (cleaned.length > 3 && cleaned.length < 200 && !constants.includes(cleaned)) {
        constants.push(cleaned);
      }
