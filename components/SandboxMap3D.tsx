...
export function SandboxMap3D() {
  ...
  const render3D = useCallback(() => {
    ...
    const vectorizedNodes = data.nodes.filter((n) => n.vector !== null) as Array<
      Node & { vector: Vector3D }
    >;
    ...
    sortedNodes.forEach((node) => {
      ...
      if (
        (selectedNode && selectedNode.submission_hash === node.submission_hash) ||
        podScore >= 8000
      ) {
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              3D Vector Map of PoC Submissions—The nested, spiraling Pong story of innovation and
              obsolescence: contributions moving from <em>unaware awareness</em> (obsolete) to{' '}
              <strong>awareness</strong> (current) to <em>meta-awareness</em> (emerging)
              {data && (
                <span className="ml-2">
          ...
          <div className="flex gap-2">
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => setCamera({ x: 0, y: 0, z: 0, rotationX: 0, rotationY: 0, zoom: 1 })}
            >
      ...
      <CardContent>
        <div className="space-y-4">
          <div className="relative rounded-lg border bg-slate-50">
            <canvas
              ref={canvasRef}
              width={800}
              height={600}
              className="h-auto w-full cursor-move"
          ...
          {selectedNode && (
            <div className="rounded-lg border bg-slate-50 p-4">
              <h3 className="mb-2 font-semibold">{selectedNode.title}</h3>
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  <span className="text-muted-foreground">Contributor:</span>{' '}
                  {selectedNode.contributor}
                </div>
                <div>
                ...
                {selectedNode.vector && (
                  <>
                    <div>
                      <span className="text-muted-foreground">X (Novelty):</span>{' '}
                      {selectedNode.vector.x.toFixed(2)}
                    </div>
                    <div>
                      <span className="text-muted-foreground">Y (Density):</span>{' '}
                      {selectedNode.vector.y.toFixed(2)}
                    </div>
                    <div>
                      <span className="text-muted-foreground">Z (Coherence):</span>{' '}
                      {selectedNode.vector.z.toFixed(2)}
                    </div>
                    <div>
