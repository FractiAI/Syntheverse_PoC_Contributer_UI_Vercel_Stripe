    ...
    try {
        ...
        if (vectorsData.vectors && vectorsData.vectors.length > 0) {
            ...
            if (firstVector.vector) {
                console.log(`     Coordinates: (${firstVector.vector.x.toFixed(2)}, ${firstVector.vector.y.toFixed(2)}, ${firstVector.vector.z.toFixed(2)})`)
            }
        }
        console.log()
    } catch (error) {
    ...
    try {
        ...
        if (mapResponse.ok) {
            ...
            if (withVectors.length > 0) {
                ...
                if (sample.vector) {
                    console.log(`     Coordinates: (${sample.vector.x.toFixed(2)}, ${sample.vector.y.toFixed(2)}, ${sample.vector.z.toFixed(2)})`)
                }
            }
        }
        console.log()
