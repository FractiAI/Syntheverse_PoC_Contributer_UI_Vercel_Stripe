...
async function analyzeGrokResponses() {
    ...
    try {
            ...
            if (contribution.length > 0 && contribution[0].metadata) {
                ...
                if (metadata.error_details || metadata.full_grok_response || metadata.raw_grok_answer) {
                    ...
                    if (metadata.full_grok_response) {
                        console.log('Full Grok Response:', JSON.stringify(metadata.full_grok_response, null, 2))
                    }
                    if (metadata.raw_grok_answer) {
                        console.log('Raw Grok Answer (first 500 chars):', metadata.raw_grok_answer.substring(0, 500))
                    }
                    console.log('\n')
                }
            }
        ...
        const withRawAnswer = failedLogs.filter(log => 
            log.response_data?.raw_grok_answer || 
            log.response_data?.full_grok_response
        ).length
