/**
 * Script to clear PoC archive for testing
 * Deletes all contributions, allocations, and related logs
 *
 * Run: npx tsx scripts/clear-poc-archive.ts
 *
...
async function clearPoCArchive() {
  try {
    ...
    const logsBefore = await db.select({ count: sql<number>`COUNT(*)` }).from(pocLogTable);
    ...
    const remainingLogs = await db.select({ count: sql<number>`COUNT(*)` }).from(pocLogTable);
