...
async function testRegistrationEndpoint() {
  ...
  if (hasSupabaseUrl && hasSupabaseAnonKey) {
    try {
      ...
      if (authData?.session) {
        ...
        logResult('Authentication', 'PASS', 'Successfully authenticated', { email: TEST_EMAIL });
      } else {
        logResult(
          'Authentication',
          'SKIP',
      ...
      logResult('Authentication', 'SKIP', 'Authentication test skipped (Supabase client error)', {
  ...
  if (hasDatabaseUrl) {
    try {
      ...
      logResult('Database Connection', 'FAIL', 'Failed to connect to database', {
        error: error instanceof Error ? error.message : 'Unknown error',
      });
    }
  } else {
    ...
    try {
      ...
      if (!isValidFormat) {
        ...
        try {
          ...
          logResult('Stripe API Connection', 'PASS', 'Successfully connected to Stripe API', {
            accountId: account.id,
            country: account.country,
            type: account.type,
          });
        } catch (stripeError: any) {
          logResult('Stripe API Connection', 'FAIL', 'Failed to connect to Stripe API', {
      ...
      logResult('Stripe Client', 'FAIL', 'Failed to initialize Stripe client', {
        error: error instanceof Error ? error.message : 'Unknown error',
      });
    }
  } else {
  ...
  if (hasDatabaseUrl) {
    try {
      ...
      logResult('Registration Route', 'PASS', 'Registration route module loaded successfully', {
        hasPostMethod: typeof registerRoute.POST === 'function',
      });
    } catch (error) {
      logResult('Registration Route', 'FAIL', 'Failed to load registration route', {
        error: error instanceof Error ? error.message : 'Unknown error',
      });
    }
  } else {
