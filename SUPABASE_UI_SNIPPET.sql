-- ============================================================================
-- BridgeSpec and TO (Testability) Integration - Copy-Paste for Supabase UI
-- Date: 2026-01-15
...
-- Check if bridge_spec column exists
SELECT column_name, data_type 
FROM information_schema.columns 
WHERE table_name = 'contributions' AND column_name = 'bridge_spec';

-- Check if indexes were created
SELECT indexname, indexdef 
FROM pg_indexes 
WHERE tablename = 'contributions' 
AND indexname LIKE 'idx_contributions_%bridge%' 
OR indexname LIKE 'idx_contributions_%bubble%'
OR indexname LIKE 'idx_contributions_%n_hat%'
OR indexname LIKE 'idx_contributions_%t_b%';
